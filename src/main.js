  ...
  functionDeclarations: [
    {
      ...
      description: "Launch a local desktop application by name (e.g. 'notepad', 'calc', 'chrome', 'paint', 'cmd', 'explorer' for File Manager).",
      parameters: {
        type: "OBJECT",
        properties: {
    ...
    {
      name: "open_folder",
      description: "Open a specific folder path in File Explorer (e.g. 'C:\\', 'workspace', or a custom path).",
      parameters: {
        type: "OBJECT",
        properties: {
          folderPath: { type: "STRING", description: "The absolute or relative directory path to open." }
        }
      }
    },
    {
    ...
    },
    {
      ...
      parameters: {
        type: "OBJECT",
        properties: {
          songName: { type: "STRING", description: "The name of the song, artist, or playlist to search and play." }
        },
        required: ["songName"]
      }
    },
    {
      ...
      description: "Execute an arbitrary shell command on the host laptop (full control access).",
      parameters: {
        type: "OBJECT",
        properties: {
...
const state = {
  ...
  neuralCore: 'ollama',  // Switchable brain (gemini or ollama) - Ollama is default
    ...
    try {
      const systemPrompt = state.language === 'ta-IN'
        ? `You are AU, a brilliant AI assistant modeled after JARVIS from Iron Man, but speaking Tamil.
           ...
           - open_application(appName) (notepad, calc, chrome, paint, cmd, explorer for File Manager)
           - open_folder(folderPath) (opens a folder path in explorer, e.g. "C:\\" or "workspace")
           - close_application(appName) (closes notepad, chrome, etc.)
           - search_web(query)
           - get_datetime()
           - set_volume(level) (0-100)
           - take_screenshot()
           - play_spotify(songName)
           - run_shell_command(command) (runs arbitrary shell commands, full control access)
           
           If the user asks you to perform one of these, you MUST output a tool call block like:
           [TOOL_CALL: name{"arg": "value"}]
           Example: [TOOL_CALL: open_application{"appName": "explorer"}]
           Example: [TOOL_CALL: open_folder{"folderPath": "C:\\"}]
           Example: [TOOL_CALL: play_spotify{"songName": "Hotel California"}]
      ...
      const res = await window.electronAPI.ollamaRequest('/api/chat', 'POST', {
...
coreGeminiBtn.addEventListener('click', () => {
  ...
  searchStatusText.innerText = 'Search: Standby';
  engineStatusText.innerText = 'Neural Engine: Gemini 2.0 Flash';
...
coreOllamaBtn.addEventListener('click', () => {
  ...
  engineStatusText.innerText = `Neural Engine: Ollama (${state.ollamaModel})`;
...
ollamaModelInput.addEventListener('change', () => {
  ...
  if (modelVal) {
    ...
    engineStatusText.innerText = `Neural Engine: Ollama (${modelVal})`;
    ...
    if (type === 'progress') {
      ...
      if (state.isRegistering) {
        window.electronAPI.saveVoiceProfile(embedding).then((res) => {
          if (res.success) {
            updateState({ ownerProfile: embedding, isRegistering: false, voiceLock: true });
            ...
            addDialogueLine('System', 'Voice profile registered successfully! Voice Lock auto-activated.');
            speak(state.language === 'ta-IN' 
              ? 'குரல் பதிவு வெற்றிகரமாக முடிந்தது ஐயா. சிஸ்டம் லாக் இப்போது தயார்.' 
              : 'Voice print registered successfully, sir. System voice lock is now active.');
          } else {
...
async function checkOllamaStatus() {
  ...
  const inputEl = document.getElementById('ollama-model-input');
  if (!selectEl || !inputEl) return;

  try {
    const res = await window.electronAPI.ollamaRequest('/api/tags');
    if (!res.success) throw new Error(res.error || 'Ollama offline');
    ...
    if (models.length > 0) {
      ...
      models.forEach(model => {
        ...
        // If it matches state.ollamaModel, select it
        if (model.name === state.ollamaModel || model.name.split(':')[0] === state.ollamaModel) {
          option.selected = true;
          state.ollamaModel = model.name; // Use exact name
        }
        selectEl.appendChild(option);
      });
      
      // Add custom option to use text input if desired
      const customOpt = document.createElement('option');
      customOpt.value = 'custom_input';
      customOpt.innerText = '[Enter custom model...]';
      ...
      selectEl.onchange = () => {
        if (selectEl.value === 'custom_input') {
          selectEl.style.display = 'none';
          inputEl.style.display = 'block';
          inputEl.focus();
        } else {
          updateState({ ollamaModel: selectEl.value });
          if (state.neuralCore === 'ollama') {
            engineStatusText.innerText = `Neural Engine: Ollama (${selectEl.value})`;
          }
          addDialogueLine('System', `Ollama model switched to: ${selectEl.value}`);
        }
      };
      
      // Update engineStatusText
      if (state.neuralCore === 'ollama') {
        engineStatusText.innerText = `Neural Engine: Ollama (${state.ollamaModel})`;
      }
    }
  } catch (err) {
    console.log('Ollama is offline or unreachable:', err);
    // Keep text input visible as fallback
    selectEl.style.display = 'none';
    inputEl.style.display = 'block';
  }
}
...
window.electronAPI.loadVoiceProfile().then((profile) => {
  if (profile) {
    updateState({ ownerProfile: profile, voiceLock: true });
    document.getElementById('security-status-text').innerText = 'Profile: Registered';
    voiceLockBtn.disabled = false;
    voiceLockBtn.innerText = 'ON';
    voiceLockBtn.classList.add('active');
...
// Greeting
const greetingText = state.language === 'ta-IN'
  ? 'வணக்கம் ஐயா. நான் AU. என்ன உதவி செய்யட்டும்?'
  : 'Good day, sir. I am AU, your personal AI assistant. Neural core is initializing.';

addDialogueLine('Au', greetingText);

// Speak the greeting automatically on app startup
...
// Listen for clap-focus event from the Electron main process
window.electronAPI.onAppFocusedByClap(() => {
  console.log('[IPC] App focused by clap detector. Starting quick voice response...');
  addDialogueLine('Au', 'Yes, sir? I am listening.');
  speak('Yes, sir? I am listening.').then(() => {
    // If transcription model is loaded, start listening
    if (state.modelReady && !state.isRecording) {
      startRecording();
    }
  });
});
